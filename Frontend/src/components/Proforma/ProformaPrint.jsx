...
function ProformaPrint({ proforma, open, onClose }) {
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-20">
      {/* Modal Container */}
      <div className="bg-white w-[70%] max-h-[95vh] overflow-y-auto rounded-lg shadow-lg p-6 relative">
        {/* Printable Section */}
        <div id="print-section" className="text-black font-sans">
          ...
          {/* Labour Table */}
          <h2 className="font-semibold mb-1">Labour</h2>
          <table className="w-full border border-collapse text-sm mb-4">
            ...
            <tbody>
              {proforma.labour_items.map((item, idx) => (
          ...
          {/* Spare Table */}
          <h2 className="font-semibold mb-1">Spare Parts</h2>
          <table className="w-full border border-collapse text-sm mb-4">
            <thead>
              <tr className="bg-gray-100 text-center">
                <th className="border px-2 py-1">No</th>
            ...
            <tbody>
              {proforma.spare_items.map((item, idx) => (
                <tr key={item.id} className="text-center">
                  <td className="border px-2 py-1">{idx + 1}</td>
                  <td className="border px-2 py-1">{item.description}</td>
                  <td className="border px-2 py-1">{item.unit || "-"}</td>

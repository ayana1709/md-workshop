...
const PendingOut = () => {
  ...
  const [pendingItemOut, setPendingItemOut] = useState([]);
  ...
  // ✅ Fetch pending item out from API
  const fetchPendingItemOut = async () => {
    setLoading(true);
    try {
      const response = await api.get("/pending-item-out"); // API endpoint
      setPendingItemOut(response.data);
    } catch (error) {
  ...
  useEffect(() => {
    if (!Array.isArray(pendingItemOut)) {
      console.error("pendingRequests is not an array:", pendingItemOut);
      return;
    }

    let result = pendingItemOut.filter((request) => {
    ...
    setCurrentPage(1);
  }, [searchTerm, startDate, endDate, pendingItemOut]);
  ...
  return (
    <div className="relative container mx-auto my-4 p-6 bg-white dark:bg-gray-800 rounded-md">
      <h2 className="text-xl font-semibold mb-4 text-blue-500 tracking-wider text-blue-500 uppercase">
        Pending Item Out
      </h2>
            ...
            {paginatedItems.length > 0 ? (
              paginatedItems.map((item) => (
                ...
                >
                  ...
                  <td className="text-center border border-table-border px-2 py-2">
              ...
              <tr>
                <td
                  colSpan="12"
                  className="border border-table-border px-4 py-2 text-center"
                >
                  No pending item out found.
                </td>
              </tr>
            )}
          </tbody>

...
const Pending = () => {
  ...
  const fetchPendingRequests = async () => {
    setLoading(true);
    try {
      const response = await api.get("/pending-requested-items");
  ...
  // ✅ Handle Item Out Action
  const handleItemOut = async (item) => {
    try {
      const response = await api.post("/pending-item-out", {
      ...
      toast.success("Item successfully moved to Pending Item Out!");
      fetchPendingRequests(); // ✅ Refresh list after action
    } catch (error) {
      console.error("Error moving item out:", error.response?.data || error);
      toast.error("Failed to move item out.");
    }
  };
  ...
  const handleReject = async (id) => {
    try {
      await api.delete(`/pending-requested-items/${id}`);
      toast.success("Item removed from pending requests.");
  ...
  useEffect(() => {
    if (!Array.isArray(pendingRequests)) {
      console.error("pendingRequests is not an array:", pendingRequests);
      return;
    }

    let result = pendingRequests.filter((request) => {
    ...
    setCurrentPage(1);
  }, [searchTerm, startDate, endDate, pendingRequests]);
  ...
  return (
    <div className="relative container mx-auto my-4 p-6 bg-white dark:bg-gray-800 rounded-md">
      <h2 className="text-xl font-semibold mb-4 uppercase text-blue-500 tracking-wider">
        ...
        <table className="min-w-full table-auto border-collapse border border-table-border">
          ...
          <tbody>
            {paginatedItems.map((item) => (
              <tr
                key={item.id}
                className="text-gray-700 dark:text-gray-200 dark:bg-gray-800 hover:dark:bg-gray-600"
              >
                ...
                <td className="border px-4 py-2">{item.unit_price || "N/A"}</td>
                <td className="border px-4 py-2">
                  {item.total_price || "N/A"}
                </td>
                <td className="border px-4 py-2"><span className="px-3 py-1 text-white rounded-md bg-yellow-500">{item.status}</span></td>
                <td className="border px-2 py-2 relative">
                  ...
                  {openDropdown === item.id && (
                    <div className="absolute z-[999] right-0 mt-2 w-40 bg-white border border-gray-300 shadow-lg rounded-md dark:bg-gray-800">
                      <button
                        className="block w-full text-left px-4 py-2 hover:bg-gray-200 hover:dark:bg-gray-600"
                        onClick={() => handleItemOut(item)}
                      >
                        Item Out
                      </button>
                      <button

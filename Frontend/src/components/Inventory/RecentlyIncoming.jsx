...
const TotalIncoming = () => {
  ...
  const fetchSpareRequests = async () => {
    setLoading(true);
    try {
      const response = await api.get("/recently-incoming");
      setSpareRequests(response.data);
    } catch (error) {
  ...
  const handleItemOut = async (request) => {
    try {
      const requestData = {
        ...
        sparedetails: JSON.parse(request.sparedetails),
      };

      console.log("Sending Item Out Request:", requestData);

      const response = await api.post("/request-item-out", requestData);
      console.log("Item Out Request Response:", response.data);

      // ✅ Re-fetch spare requests to get updated data
      fetchSpareRequests();
      ...
      alert("Failed to process item out request.");
    }
  };
  ...
  const handleAddToPending = async (request) => {
    try {
      ...
      alert("Item successfully added to Pending Requests!");
    } catch (error) {
      console.error(
        "Error adding item to pending:",
        error.response?.data || error
      );
      alert("Failed to add item to pending.");
    }
  };
  ...
  return (
    <div className="relative z-[9] container mx-auto my-4 p-6 bg-white dark:bg-gray-800 rounded-md">
      ...
      <div ref={printRef} className="mt-16">
        <table className="min-w-full table-auto border-collapse border border-table-border">
          <thead>
            <tr className="text-white">
              <th className="border px-4 py-2 bg-table-head">ID</th>
              <th className="border px-4 py-2 bg-table-head">Description</th>
              <th className="border px-4 py-2 bg-table-head">Part Number</th>
              <th className="border px-4 py-2 bg-table-head">Brand</th>
              <th className="border px-4 py-2 bg-table-head">Model</th>
              <th className="border px-4 py-2 bg-table-head">Quantity</th>
              <th className="border px-4 py-2 bg-table-head">Status</th>
              <th className="border px-4 py-2 bg-table-head">Unit Price</th>
              <th className="border px-4 py-2 bg-table-head">Total Price</th>
              <th className="border px-4 py-2 bg-table-head">Action</th>
            </tr>
          </thead>
          <tbody>
            {paginatedItems.map((request) => {
              ...
              return (
                <tr
                  key={request.id}
                  className="text-grya-700 dark:text-gray-200 hover:dark:bg-gray-600"
                >
                  ...
                  <td className="border px-4 py-2">
                   <p className={`px-3 py-1 text-white rounded-md ${spareDetails[0]?.status==="Available"?"bg-green-500":"bg-red-500"}`}> {spareDetails[0]?.status || "N/A"}</p>
                  </td>
                  <td className="border px-4 py-2">

  ...
  const importItems = async (file) => {
    ...
    try {
      ...
      if (!jsonData.length) {
        throw {
          type: "client",
          message: "The selected Excel file contains no data.",
        };
      }

      // 2️⃣ Send to backend
      const res = await api.post("/items/import", {
        items: jsonData,
      });

      Swal.close();

      const { inserted = 0, failed = 0, errors = [] } = res.data;
      ...
      if (failed === 0) {
        await Swal.fire({
          ...
          text: `${inserted} items imported successfully.`,
        });
      }
      // 4️⃣ Partial success
      else {
        const errorList = errors
          .slice(0, 8) // prevent huge modal
        ...
        await Swal.fire({
          ...
          html: `
          <p><b>${inserted}</b> items imported successfully.</p>
          <p><b>${failed}</b> rows failed.</p>
          <hr/>
          <div style="text-align:left;font-size:13px">
            ${errorList}
            ${errors.length > 8 ? "<br/>…and more" : ""}
          </div>
        `,
        });
      }
      ...
      // 5️⃣ Client-side error
      if (err?.type === "client") {
        Swal.fire("Invalid File", err.message, "error");
        return;
      }

      // 6️⃣ Backend validation error (422)
      if (err.response?.status === 422) {
        Swal.fire(
          "Invalid Data",
          "The uploaded file has an invalid structure.",
          "error",
        );
        return;
      }
      ...
      Swal.fire(
        ...
        err.response?.data?.message ||
          "Unable to import items. Please try again.",
        "error",
      );
    } finally {
      setImporting(false);
  ...
  return (
    <div className="p-4 sm:p-6 bg-white dark:bg-gray-800 dark:text-white rounded-lg shadow-md">
      <h2 className="text-lg sm:text-xl font-bold text-green-500 mb-4">
        Total items list / ጠቅላላ የዕቃ ዝርዝር
      </h2>

...
const OutSourceList = () => {
  ...
  useEffect(() => {
    const fetchRepairs = async () => {
      try {
        const response = await api.get("/outsource");
        ...
        const merged = Object.values(
          fetchedData.reduce((acc, curr) => {
            ...
            if (!acc[key]) {
              // First time seeing this job_card_no
              acc[key] = {
                ...curr,
                outsourcedetails: [...(curr.outsourcedetails || [])],
              };
            } else {
              // Merge outsourcedetails into same job_card_no
              acc[key].outsourcedetails.push(...(curr.outsourcedetails || []));
            }
        ...
        console.error("Error fetching outsourced data:", error);
        setWork([]);
      }
    };
                ...
                <table className="table-fixed min-w-full table-auto w-full border-collapse border border-table-border overflow-x-auto">
                  <thead className="bg-table-head text-white text-left text-sm">
                    <tr className="w-full border-table-border py-2">
                      ...
                      <th className="border border-table-border p-2 w-[950px]">
                        Out Source
                      </th>
                      ...
                      displayedRepairs.map((repair) => (
                        ...
                        >
                          ...
                          <td className="px-4 border border-table-border py-3">
                            ...
                            {repair.repair_category.length > 2 && "..."}
                          </td>

                          <td className="px-4 border border-table-border py-3">
                            {repair.outsourcedetails &&
                            repair.outsourcedetails.length > 0 ? (
                              <table className="table-fixed min-w-full table-auto w-full border-collapse border border-table-border overflow-x-auto">
                                <thead className="bg-table-head text-white text-left text-sm">
                                  <tr className="">
                                ...
                                <tbody>
                                  {repair.outsourcedetails &&
                                  Array.isArray(repair.outsourcedetails)
                                    ? repair.outsourcedetails.map(
                                        (detail, index) => (
                                          <tr
                                            key={index}
